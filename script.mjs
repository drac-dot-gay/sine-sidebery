...
if (sidebery_policy) {
  ...
  sidebery_policy.baseCSP =
    "script-src 'self' https://* http://localhost:* http://127.0.0.1:* moz-extension: chrome: blob: filesystem: 'unsafe-eval' 'wasm-unsafe-eval' 'unsafe-inline' chrome:;";
...
async function setupSideberyPanel(win) {
  ...
  win.sidebery_browser.setAttribute(
    "initialBrowsingContextGroupId",
    sidebery_policy.browsingContextGroupId,
  );
...
function loadSideberyPanel(win) {
  ...
  win.sidebery_browser.loadURI(Services.io.newURI(sidebery_url), {
    triggeringPrincipal,
  });
...
const fixNoGrabbingCursorOnDrag =
  // attempt to fix bug #3
  `
...
const fixInheritBadBrowserStyles =
  // some zen/ff styles make things worse, put it back
  `
    :root {
...
const handleCompactMode =
  //collapsed toolbar goes down to 48px - hide nesting
  `
...
allStyleMods = [
  getZenCSSVariables(),
  fixNoGrabbingCursorOnDrag,
  transparentByDefault,
  fixInheritBadBrowserStyles,
  zenStylesByDefault,
  handleCompactMode,
];
...
function afterSideberyLoads(win) {
  ...
  const zenStylesheets = [...win.document.styleSheets].map((styleSheet) => {
    return styleSheet.href;
  });
  const allStyleModsAsDataURLs = allStyleMods.map(
    (css) => `data:text/css,${encodeURIComponent(css)}`,
  );

  let stylesheets = [
    ...zenStylesheets,
    "chrome://browser/content/extension.css",
    ...allStyleModsAsDataURLs,
  ].filter((sheet) => sheet); //discard nulls
  console.log(stylesheets);
  win.sidebery_browser.messageManager.sendAsyncMessage(
      ...
      screens: [
        {
          ...
          content: {
            logo: {
              imageURL:
                "https://raw.githubusercontent.com/mbnuqw/sidebery/v5/docs/assets/readme-logo.svg",
              height: "73px",
            },
            has_noodles: true,
            title: {
